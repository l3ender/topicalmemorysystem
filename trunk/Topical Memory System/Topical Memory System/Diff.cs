...
namespace Topical_Memory_System
{
	class Diff
	{
		...
		public static ComparedString DiffText(string TextA, string TextB)
		{
			int[] a_codes = Diff.DiffCharCodes(TextA, true);
			int[] b_codes = Diff.DiffCharCodes(TextB, true);
			Item[] diffs = Diff.DiffInt(a_codes, b_codes);
			...
			for (int n = 0; n < diffs.Length; n++)
			{
				...
				while ((pos < it.StartB) && (pos < TextB.Length))
				{
					result.Append(TextB[pos]);
					pos++;
				}
				...
				if (it.deletedA > 0)
				{
					Selection p = new Selection(Color.Green);
					p.Start = result.Length;
					for (int m = 0; m < it.deletedA; m++)
					{
						result.Append(TextA[it.StartA + m]);
					}
					p.End = result.Length;
					selections.Add(p);
				}
				...
				{
					Selection p = new Selection(Color.Red);
					p.Start = result.Length;
			...
			while (pos < TextB.Length)
			{
				result.Append(TextB[pos]);
				pos++;
			}

			return (new ComparedString(selections, result.ToString()));
		}
