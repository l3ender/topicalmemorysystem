...
namespace Topical_Memory_System
{
	...
	public class VisualCue
	{
		...
		public void Draw(int itemIndex)
		{
			...
			if (_listBox.Sorted)
			{ // Draw vertical cue if the list is sorted, since items could just be dropped anywhere.
			...
			{ // Draw horizontal line.
				...
				{ // No current item available. Get the client rectangle instead.
					rect = _listBox.ClientRectangle;
				}
				else if (itemIndex < _listBox.Items.Count)
				{ // Somewhere in the list, this is our current item.
					rect = _listBox.GetItemRectangle(itemIndex);
				}
				else
				{ // Somewhere after the last item. Let's draw the visual cue just after the last item in the list.
				...
				if (rect.Y < _listBox.ClientRectangle.Y)
				{ // Make sure we draw inside of the client rectangle.
